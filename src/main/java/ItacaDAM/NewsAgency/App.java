...
public class App {
	...
	public static List<News> newsCreator(int n) {
		...
		for (int i = 0; i < n; i++) {
			...
			switch (random.ints(0, 2).findFirst().getAsInt()) {
			case 0:
				news.add(new FootballNews("Football match ended!"));
				break;
			case 1:
				news.add(new RugbyNews("Great Try!"));
				break;
			case 2:
				news.add(new HandballNews("2 minutes and 7 meters for the team!"));
				break;
			}
			;
		}
